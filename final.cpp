...
void SDN_switch::recv_handler(packet* p) {
    ...
    if (p == nullptr) return;
    ...
    else if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        // cout << "node " << getNodeID() << " send the packet" << endl;
        SDN_data_packet* p2 = nullptr;
        p2 = dynamic_cast<SDN_data_packet*> (p);
        if (p2->getHeader()->getDstID() == getNodeID())return; // if dest does not exist or dest == current id do nothing
        p2->getHeader()->setPreID(getNodeID()); // get current node id
        p2->getHeader()->setNexID(table[p2->getHeader()->getDstID()]); //get next node of dest from map and set to the header
        ...
        table[mat] = act;
        unsigned int srcID = p3->getHeader()->getSrcID(); // prepare packet for acknowledgement
    ...
    else if (p->type() == "SDN_stat_packet") { //send stat packet to controller
...
void SDN_controller::recv_handler(packet* p) {
    ...
    if (p == nullptr) return;
    else if (p->type() == "SDN_ctrl_new_packet" && p->getHeader()->getDstID() == getNodeID()) { //ack for initail table, do nothing
        return;
    }
    else if (p->type() == "SDN_ctrl_new_packet") { // the controller receives initial ctrl packet
        SDN_ctrl_new_packet* p3 = nullptr;
        p3 = dynamic_cast<SDN_ctrl_new_packet*> (p);
        SDN_ctrl_new_payload* l3 = nullptr;
        l3 = dynamic_cast<SDN_ctrl_new_payload*> (p3->getPayload());
...
int main() {
    ...
    unsigned int nodeCnt, nodeCost;
    unsigned int srcId, dstId, oldPathLen;
    int insTime, updTime, simTime, dataTime;
    unsigned dataSrc, dataDst;
    vector<OldPath>oldPath;
    vector<Link>newLink;
    map<unsigned int, vector<pair<unsigned int, unsigned int>>>nodes; // nodes and their neighbors


    unsigned int linkCnt;

    // read the input and generate switch nodes
    cin >> nodeCnt >> nodeCost;
    cin >> insTime >> updTime >> simTime;

    cin >> srcId >> dstId >> oldPathLen;

    for (auto i = 0; i < oldPathLen; i++) {
        unsigned int id, node1, node2;
        cin >> id >> node1 >> node2;
        oldPath.push_back({ node1, node2 }); // save old path
    }

    cin >> linkCnt;
    for (auto i = 0; i < linkCnt; i++) { // save link weight and neighbors
        unsigned int id, node1, node2, newLinkWeight;
        cin >> id >> node1 >> node2 >> newLinkWeight;
        newLink.push_back({ node1, node2, newLinkWeight });
        nodes[node1].push_back({ node2, newLinkWeight });
        nodes[node2].push_back({ node1, newLinkWeight });
    }
    ...
    for (auto i : nodes) {
        for (auto j : i.second) {
            node::id_to_node(i.first)->add_phy_neighbor(j.first);
        }
    }
    ...
    while (cin >> dataTime >> dataSrc >> dataDst) {
        data_packet_event(dataSrc, dataDst, dataTime);
    }

    for (auto i : oldPath) {
        ctrl_new_packet_event(con_id, i.node1, dstId, i.node2, insTime);
    }

    map<unsigned int, map<unsigned int, unsigned int> >link_weight;
    for (auto i : nodes) { // modify weight structure for each switch
        for (auto j : i.second) {
            link_weight[i.first][j.first] = j.second;
            // stat_packet_event(i.first, con_id, { j.first, j.second }, 0);
        }
    }
    for (auto i : link_weight) {
        stat_packet_event(i.first, con_id, i.second, updTime);
    }
    // for (auto i : nodes) {
