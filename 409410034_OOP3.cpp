...
void SDN_controller::recv_handler(packet* p) {
    ...
    else if (p->type() == "SDN_ctrl_packet") { // the controller receives a packet from the controller
        SDN_ctrl_packet* p3 = nullptr;
        p3 = dynamic_cast<SDN_ctrl_packet*> (p);
        SDN_ctrl_payload* l3 = nullptr;
        l3 = dynamic_cast<SDN_ctrl_payload*> (p3->getPayload());
...
void dijkstra(map<unsigned int, unsigned int>& table, unsigned int dest, map<unsigned int, Node>& nodes, int nodeCnt, bool newWeight) {
...
// bool findConflict(vector<Round>roundA, vector<Round>roundB, map<unsigned int, unsigned int>& table) {
//     for (auto elemA : roundA) {
//         for (auto elemB : roundB) {
//             if (table[elemA.target] == elemB.target) {
//                 return true;
//             }
//         }
//     }
...
// void removeNoChange(map<unsigned int, unsigned int>& table, map<unsigned int, unsigned int>& newTable) {
//     for (auto i : table) {
//         if (i.second == newTable[i.first]) {
//             newTable.erase(i.first);
//         }
//     }
// }
void calRound(map<unsigned int, unsigned int>& table, map<unsigned int, unsigned int>& newTable, unsigned int dest, vector < vector<Round > >& round) {
    ...
    //     for (size_t j = i + 2; j < round.size(); j++) {
    //         if (!findConflict(round[i], round[j], table)) {
    //             round[i].insert(round[i].end(), round[j].begin(), round[j].end());
    //             auto it = round.begin() + j;
    //             round.erase(it);
    //             return;
    //         }
    //     }
...
int main() {
    ...
    map<unsigned int, vector<vector<Round > > >round; // update round of the route table
    map<unsigned int, vector<vector<Round > > >newRound; // update round of new route table
    ...
    // for (unsigned int i = 0; i < destCnt; i++) {
    //     removeNoChange(tableList[i], newTableList[i]);
    // }
    ...
    controller->setNewRound(newRound); // set round to controller
    ...
    for (unsigned int id = 0; id < node::getNodeNum(); id++) {
        for (unsigned int i = 0; i < destCnt; i++) {
            ...
            ctrl_packet_event(con_id, id, destIds[i], tableList[i][id], insTime);
        }
    }
    // set first round of updated routing table
    for (auto i : destIds) {
        for (auto k : newRound[i][0]) {
            ctrl_packet_event(con_id, k.target, k.match, k.action, updTime, "update");
        }
    }
