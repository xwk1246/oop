...
public:
    ...
    vector<pair<unsigned int, unsigned int> > getNeighbors() { return neighbors; }
    void setNeighbors(vector<pair<unsigned int, unsigned int> > _neighbors) { neighbors = _neighbors; }
    ...
    class SDN_stat_payload_generator : public payload_generator {
        ...
        // this constructor is only for sample to register this payload type
        SDN_stat_payload_generator() { /*cout << "SDN_ctrl_payload registered" << endl;*/ register_payload_type(&sample); }
    protected:
        virtual payload* generate() {
            // cout << "SDN_ctrl_payload generated" << endl;
...
public:
    ...
    class SDN_stat_packet_generator : public packet_generator {
        ...
        // this constructor is only for sample to register this packet type
        SDN_stat_packet_generator() { /*cout << "SDN_ctrl_packet registered" << endl;*/ register_packet_type(&sample); }
    protected:
        virtual packet* generate(packet* p = nullptr) {
            // cout << "SDN_ctrl_packet generated" << endl;
            if (nullptr == p)
...
struct Round {
    ...
    unsigned int action;
    unsigned int state;
    bool operator==(const Round& a) { return a.target == this->target; }
};
...
class SDN_controller : public node {
    ...
    map<unsigned int, vector<vector<Round > > >newRound; // update round of the route table
protected:
...
public:
    map<unsigned int, unsigned int> newCurRound;
    ...
    vector<unsigned int> statReceived;
    map<unsigned int, vector<pair<unsigned int, unsigned int>>> nodes; // nodes list and their neighbor
    vector<unsigned int>destIds;

    ~SDN_controller() {}
    string type() { return "SDN_controller"; }
    void setNewRound(map<unsigned int, vector<vector<Round > > > _round) {
        newRound = _round;
    }
    // please define recv_handler function to deal with the incoming packet
    virtual void recv_handler(packet* p);
...
unsigned int SDN_stat_pkt_gen_event::event_priority() const {
    ...
    for (auto i : neighbors) {
        string_for_hash += to_string(i.first) + to_string(i.second);
    }
    return get_hash_value(string_for_hash);
}
// the SDN_ctrl_pkt_gen_event::print() function is used for log file
...
void stat_packet_event(unsigned int con_id, unsigned int id, vector<pair<unsigned int, unsigned int> >neighbors, unsigned int t = event::getCurTime(), string msg = "default") {
    // void ctrl_packet_event (unsigned int dst, unsigned int t = 0, string msg = "default"){
    if (id == BROCAST_ID || node::id_to_node(id) == nullptr) {
        cerr << "id is incorrect" << endl; return;
    }
...
void SDN_switch::recv_handler(packet* p) {
    ...
    if (p->type() == "SDN_data_packet") { // the switch receives a packet from the other switch
        ...
        set_next_node_of_dest(mat, act); // set map of switch
        unsigned int srcID = p3->getHeader()->getSrcID(); // prepare packet for acknoledgement
        unsigned int dstID = p3->getHeader()->getDstID();
        unsigned int preID = p3->getHeader()->getSrcID();
        unsigned int nexID = p3->getHeader()->getNexID();
        p3->getHeader()->setSrcID(dstID);
        p3->getHeader()->setDstID(srcID);
        p3->getHeader()->setNexID(srcID);
        p3->getHeader()->setPreID(dstID);
        send_handler(p3); // send back to controller
    }
...
class Node;
void dijkstra(map<unsigned int, unsigned int>& table, unsigned int dest, map<unsigned int, vector<pair<unsigned int, unsigned int>>>& nodes, int nodeCnt);
void calRound(map<unsigned int, unsigned int>& table, map<unsigned int, unsigned int>& newTable, unsigned int dest, vector < vector<Round > >& round);
void SDN_controller::recv_handler(packet* p) {
    if (p == nullptr) return;

    if (p->type() == "SDN_data_packet") { // the controller receives a packet from the other switch
        return;
    }
    // when receiving ack packet generate next round update
    else if (p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() == getNodeID() && dynamic_cast<SDN_ctrl_payload*> (dynamic_cast<SDN_ctrl_packet*> (p)->getPayload())->getMsg() == "update") { //ack
        ...
        auto mat = l2->getMatID();
        auto nowRound = newCurRound[mat];
        // if (newCurRound[mat] >= newRound[mat].size())return;
        for (long unsigned int i = 0; i < newRound[mat][newCurRound[mat]].size(); i++) {
            if (newRound[mat][nowRound][i].target == p2->getHeader()->getSrcID()) { // find and update packet acknowledge state to 1
                newRound[mat][nowRound][i].state = 1;
                auto zeroFound = count_if(newRound[mat][newCurRound[mat]].begin(), newRound[mat][newCurRound[mat]].end(), [](Round r1) { return r1.state == 0; }); // if last round is all acknowledged, then generate next round
                if (zeroFound == 0) { // if all acknowledged
                    newCurRound[mat]++; // cur round ++
                    if (newCurRound[mat] >= newRound[mat].size())return; // if no next round, then return
                    for (auto j : newRound[mat][newCurRound[mat]]) {
                        ctrl_packet_event(getNodeID(), j.target, j.match, j.action, event::getCurTime(), "update"); // generate next round update packet
                    }
                }
            }
        }
    }
    else if (p->type() == "SDN_ctrl_packet" && p->getHeader()->getDstID() == getNodeID()) { //ack for initail table, do nothing
        return;
    }
    else if (p->type() == "SDN_ctrl_packet") { // the controller receives initial ctrl packet
        SDN_ctrl_packet* p3 = nullptr;
        p3 = dynamic_cast<SDN_ctrl_packet*> (p);
        SDN_ctrl_payload* l3 = nullptr;
        l3 = dynamic_cast<SDN_ctrl_payload*> (p3->getPayload());
    ...
    else if (p->type() == "SDN_stat_packet") { // receive stat packet
        SDN_stat_packet* p4 = nullptr;
        p4 = dynamic_cast<SDN_stat_packet*> (p);
        SDN_stat_payload* l4 = nullptr;
        l4 = dynamic_cast<SDN_stat_payload*> (p4->getPayload());
        nodes[p4->getHeader()->getSrcID()] = l4->getNeighbors();
        if (auto cnt = count(statReceived.begin(), statReceived.end(), 0)) // count if state 0 exists then is insert ctrl packet
            statReceived[p4->getHeader()->getSrcID()] = 1;
        else
            statReceived[p4->getHeader()->getSrcID()] = 3; // else is update packet
        if (count(statReceived.begin(), statReceived.end(), 1) == getNodeNum() - 1) { // if all state == 1
            for (unsigned int i = 0; i < destIds.size(); i++) {
                dijkstra(tableList[i], destIds[i], nodes, getNodeNum() - 1); // shortest path algorithm, using old weight
            }
            for (unsigned int id = 0; id < node::getNodeNum() - 1; id++) {
                for (unsigned int i = 0; i < destIds.size(); i++) {
                    if (id == destIds[i])continue;
                    ctrl_packet_event(getNodeID(), id, destIds[i], tableList[i][id]); // generate all insert ctrl packet
                }
            }
            for (auto& j : statReceived) {
                j = 2; // change all state to 2, means insert finished
            }
        }
        else if (count(statReceived.begin(), statReceived.end(), 3) == getNodeNum() - 1) { // if all state == 3
            for (unsigned int i = 0; i < destIds.size(); i++) {
                dijkstra(newTableList[i], destIds[i], nodes, getNodeNum() - 1); // shortest path algorithm, using new weight
                calRound(tableList[i], newTableList[i], destIds[i], newRound[destIds[i]]); // calculate round
            }
            for (auto i : destIds) {
                for (auto k : newRound[i][0]) {
                    ctrl_packet_event(getNodeID(), k.target, k.match, k.action, event::getCurTime(), "update"); // generate only first round
                }
            }
            for (auto& j : statReceived) {
                j = 4; // update finished
...
void dijkstra(map<unsigned int, unsigned int>& table, unsigned int dest, map<unsigned int, vector<pair<unsigned int, unsigned int>>>& nodes, int nodeCnt) {
    ...
    table[dest] = dest;
    selected[dest] = true;
    for (auto i : nodes[dest]) {
        oldPq.push({ dest, i.first, i.second }); // push all neighbors of dest
    }
    while (!oldPq.empty()) {
        ...
        for (auto i : nodes[current.to]) { // push all neighbors of the popped node
            if (!selected[i.first]) { // check if not selected
                oldPq.push({ current.to, i.first, i.second + (int)cost[current.to] });
            }
        }
    }
...
void calRound(map<unsigned int, unsigned int>& table, map<unsigned int, unsigned int>& newTable, unsigned int dest, vector < vector<Round > >& round) {
    // calculate depth for table and set round
    unsigned maxDepth = 0;
    map<unsigned int, unsigned int> depth;
    depth[dest] = 0;
    for (auto i : newTable) {
        unsigned int depthCnt = 0;
        unsigned int currentNode;
        currentNode = i.first;
        while (currentNode != dest) {
            depthCnt++;
            currentNode = newTable[currentNode];
        }
        depth[i.first] = depthCnt;
        if (depthCnt > maxDepth)maxDepth = depthCnt;
    }
    round.resize(maxDepth);
    for (auto i : depth) {
        if (i.first == dest)continue;
        round[i.second - 1].push_back({ i.first, dest, newTable[i.first], 0 });
    }
...
int main() {
    ...
    map<unsigned int, vector<pair<unsigned int, unsigned int> > > oldNeighbor; // neighbors of node with old weight
    map<unsigned int, vector<pair<unsigned int, unsigned int> > > newNeighbor; // neighbors of node with new weight
    ...
    for (auto i : links) { // build node list and their neighbors
        ...
        oldNeighbor[i.nodeA].push_back({ i.nodeB, i.oldWeight }); // for stat packet
        oldNeighbor[i.nodeB].push_back({ i.nodeA, i.oldWeight });
        newNeighbor[i.nodeA].push_back({ i.nodeB, i.newWeight });
        newNeighbor[i.nodeB].push_back({ i.nodeA, i.newWeight });
    }
    ...
    // generate and set neighbor of controller
    ...
    controller->tableList.resize(destCnt); // initialize table size by destCnt for controller
    controller->newTableList.resize(destCnt);
    controller->destIds = destIds; // set dest to controller
    controller->statReceived.resize(nodeCnt);

    unsigned int t, src, dst; // data packet
    while (cin >> t >> src >> dst) {
        data_packet_event(src, dst, t);
    }

    for (unsigned int i = 0; i < nodeCnt; i++) { // generate stat packet
        stat_packet_event(con_id, i, oldNeighbor[i], insTime);
        stat_packet_event(con_id, i, newNeighbor[i], updTime);
    }
